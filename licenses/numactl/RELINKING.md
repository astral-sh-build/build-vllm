...
modified compatible build of libnuma. One practical route is to unpack the wheel,
replace the auditwheel-managed `libnuma` shared object under the package's
`.libs` directory with an ABI-compatible build, and repack the wheel. Recipients
may also rebuild vLLM from source against their chosen libnuma build.
